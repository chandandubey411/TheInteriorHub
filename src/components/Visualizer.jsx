...
export default function Visualizer({ product, variant = null }) {
  ...
  const imgs = variant && product.views && product.views[variant]
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === "ArrowRight") setMainIndex((i) => Math.min((imgs.length || 1) - 1, i + 1));
    }
        ...
        {imgs && imgs.length > 0 && (
          <div className="mt-3 grid grid-cols-4 gap-2">
            {imgs.map((t, idx) => (
              <button
                key={t + idx}
                onClick={() => { setMainIndex(idx); setIsModalOpen(true); }}
                className="overflow-hidden rounded-md border hover:shadow transition"
                aria-label={`Open view ${idx + 1}`}
              >
                <img src={t} alt={`${product.title} thumb ${idx + 1}`} className="w-full h-20 object-cover" loading="lazy" />
              </button>
            ))}
          </div>
        )}
        ...
        {isModalOpen && imgs && imgs.length > 0 && (
          ...
          >
            ...
            <div className="max-w-4xl w-full">
              <img src={imgs[mainIndex]} alt={`${product.title} large ${mainIndex+1}`} className="w-full h-[70vh] object-contain rounded shadow-lg bg-white" />
      ...
      {isModalOpen && imgs && imgs.length > 0 && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/70" onClick={(e)=>{ if (e.target === e.currentTarget) setIsModalOpen(false); }}>
          <div className="relative max-w-5xl w-full">
            ...
            <img src={imgs[mainIndex]} alt={`${product.title} large ${mainIndex+1}`} className="w-full h-[80vh] object-contain rounded" />
            <div className="mt-3 flex items-center justify-center gap-3">
              <button onClick={() => setMainIndex(i => Math.max(0, i-1))} className="px-3 py-2 bg-white rounded">←</button>
